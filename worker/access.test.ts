...
const request = (origin = 'https://tintocellar.com', body = 'challenge') => new Request('https://tintocellar.com/api/proof-access', { method: 'POST', headers: { Origin: origin, 'Content-Type': 'text/plain' }, body })
...
it('rejects missing/malformed proof credentials before reading uploads or calling Images', async () => {
  ...
  for (const authorization of ['', 'Bearer fake', 'Bearer ' + 'a'.repeat(65)]) {
    const req = new Request('https://tintocellar.com/api/proof', { method: 'POST', headers: { Authorization: authorization }, body: 'unread' })
