...
it('excludes expired sources and cleans records without postponing the alarm on each import', async () => {
  ...
  expect(state.data).toEqual(new Map([['report-count-v2', 0]]))
})
...
it('explicitly migrates retained legacy records and only marks completion after successful writes', async () => {
  ...
  state.data.set('report-count-v2', 3)
  ...
  expect(state.data.get('report-count-v2')).toBe(3)
  fail = false
  ...
  expect(state.data.get('report-count-v2')).toBe(3)
  await state.object.alarm()
  expect(state.data.get('report-count-v2')).toBe(2)
})
...
it.each([0, 999])('rebuilds stale v1 count %i once and performs no warm admission scans', async staleCount => {
  const state = setup()
  state.data.set('report-count-v1', staleCount)
  ...
  expect(state.data.get('report-count-v2')).toBe(2)
  expect(state.data.get('report-count-v1')).toBe(staleCount)
...
it('enforces retained capacity under concurrent insertion without counting duplicates', async () => {
  ...
  expect(state.data.get('report-count-v2')).toBe(1000)
...
it('fails closed for invalid retained counters and unexpected oversized legacy storage', async () => {
  for (const count of [null, '1', -1, 1.5, 1001]) {
    const state = setup()
    state.data.set('report-count-v2', count)
  ...
  expect(state.data.has('report-count-v2')).toBe(false)
})
it('detects counter drift during retention without deleting reports', async () => {
  const state = setup()
  state.data.set('report-count-v2', 0)
...
it('rolls back expired deletions if updating their count fails', async () => {
  ...
  const originalPut = state.storage.put.bind(state.storage)
  vi.spyOn(state.storage, 'put').mockImplementation(async (key, value) => {
    if (key === 'report-count-v2') throw new Error('Interrupted count write')
    await originalPut(key, value)
  })
