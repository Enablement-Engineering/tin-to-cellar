import type { GalleryLabelDraft, GalleryReviewRecord, GalleryState } from '../../src/lib/gallery/types'
import { galleryIdentity, uuid } from '../../src/lib/gallery/schema'
...
export async function reviewRecord(db:GalleryDatabase,row:StoredReviewRow,now:Date):Promise<GalleryReviewRecord>{
 ...
 const metadata:GalleryLabelDraft|null=readable&&row.metadata_json?JSON.parse(row.metadata_json):null
 ...
 return{id:row.id,state:row.state,version:row.row_version,createdAt:row.created_at,expiresAt:row.expires_at,deletionDue:row.deletion_due,digest:row.digest,metadata,publicationId:row.publication_id,maker:tobacco?.maker??(metadata && galleryIdentity(metadata)?.maker)??null,blend:tobacco?.blend??(metadata && galleryIdentity(metadata)?.blend)??null,mappingNeeded:!row.catalog_id,canonicalHash:readable?row.artwork_hash:null,metadataHash:readable?row.metadata_hash:null,uploadedHash:metadata?.image.sha256??null,publishedIdentity:row.published_maker&&row.published_blend?{maker:row.published_maker,blend:row.published_blend}:null,validation:{format:'gallery-v2',geometry:'circle-2.5',imageValidated:!!row.artwork_hash,visualReviewRequired:true}}
}
...
export async function humanQueue(db:GalleryDatabase,url:URL,now:Date){
 ...
 AND (?='' OR (CASE WHEN s.state='published' THEN coalesce(s.published_maker,c.maker,json_extract(s.metadata_json,'$.tobacco.maker'),'') ELSE coalesce(c.maker,json_extract(s.metadata_json,'$.tobacco.maker'),'') END) LIKE ? ESCAPE '\\' OR (CASE WHEN s.state='published' THEN coalesce(s.published_blend,c.blend,json_extract(s.metadata_json,'$.tobacco.blend'),'') ELSE coalesce(c.blend,json_extract(s.metadata_json,'$.tobacco.blend'),'') END) LIKE ? ESCAPE '\\')
