...
export async function humanQueue(db:GalleryDatabase,url:URL,now:Date){
 ...
 AND (?='' OR coalesce(c.maker,json_extract(s.metadata_json,'$.tobacco.maker'),'') LIKE ? ESCAPE '\\' OR coalesce(c.blend,json_extract(s.metadata_json,'$.tobacco.blend'),'') LIKE ? ESCAPE '\\')
