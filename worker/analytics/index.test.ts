...
function request(body: unknown = payload(), headers: Record<string, string> = {}, method = 'POST') {
  return new Request(`https://tintocellar.com${PRINT_INTENT_PATH}`, { method, ...(method === 'POST' ? { body: JSON.stringify(body && typeof body === 'object' ? { version: 2, ...body } : body) } : {}), headers: { Origin: 'https://tintocellar.com', 'Content-Type': 'application/json', 'CF-Connecting-IP': '203.0.113.5', ...headers } })
}
...
describe('SQLite rollups and admission', () => {
  it('separates add, selection, quantity, per-blend jobs and whole jobs', async () => {
    for (const event of ['added-to-labels', 'selected-for-print', 'print-job-requested']) expect((await analyticsResponse(request(payload(event, [label, { catalogId: 'blend-b', quantity: 3 }])), env, now)).status).toBe(200)
    expect(rows()).toEqual([
    ...
    expect(db.sql.prepare('SELECT * FROM print_intent_admission').all()).toEqual([{ id: 1, period_start: '2026-09-12', admissions: 3, allowance: 1000 }])
  })
  it('resolves aliases, rejects unknown/inactive IDs and duplicate canonical identities', async () => {
    expect((await analyticsResponse(request(payload('selected-for-print', [{ catalogId: 'old-a', quantity: 1 }])), env, now)).status).toBe(200)
  ...
  it('accepts the maximum 100 canonical IDs and a 450-quantity job atomically', async () => {
    ...
    expect((await analyticsResponse(request(payload('print-job-requested', labels)), env, now)).status).toBe(200)
  ...
  it('strictly limits concurrent callers using the atomic SQLite reservation', async () => {
    ...
    expect(result.filter(item => item.status === 200)).toHaveLength(7)
  ...
  it('never moves its daily counter backward when old requests finish after midnight', async () => {
    ...
    expect(db.sql.prepare('SELECT * FROM print_intent_admission').all()).toEqual([{ id: 1, period_start: '2026-09-13', admissions: 1, allowance: 1 }])
  })
  ...
  it('stores only aggregate fields and counts repeated requests as separate actions', async () => {
    for (let i = 0; i < 2; i++) expect((await analyticsResponse(request(), env, now)).status).toBe(200)
