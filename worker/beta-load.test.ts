...
beforeEach(() => {
  ...
  for (let n = 0; n < 240; n++) {
    ...
    for (const kind of ['thumbnail', 'artwork', 'pack']) db.raw.prepare('INSERT INTO gallery_assets(id,submission_id,kind,r2_key,sha256,bytes) VALUES(?,?,?,?,?,?)').run(`${n}-${kind}`, id(n), kind, `${n}/${kind}`, hash, bytes.length)
  }
