...
The dev/build scripts prepare the OCR runtime automatically. `npm run dev` starts the complete local stack at `http://127.0.0.1:43928`: Vite with live frontend updates, the real Worker API, local gallery and diagnostics databases, artwork storage, and catalog contributions. It applies local migrations and seeds catalog identities before opening the frontend. Keep that command running while using the app. Both servers stop together on Ctrl-C or a Worker failure, and an occupied browser port produces an error instead of silently moving your browser storage to another origin.

Local data persists across starts in `.wrangler/dev`, or the directory remembered in `.wrangler/local-dev.json`. To adopt an existing populated gallery once, run `npm run dev -- --reuse .wrangler/gallery-EXAMPLE`; subsequent `npm run dev` starts reuse it automatically. `npm run gallery:dev` is an alias for the complete stack. See [gallery operations](docs/gallery-operations.md) to copy public production artwork into local storage. Startup does not download production data. Submission/reviewer authentication remains disabled or unavailable unless configured; no production resources or secrets are used.

Frontend edits update live, and `npm run build` can run independently without invalidating open development tabs. `npm run preview` remains a built-site Wrangler preview using the root configuration; it does not initialize the complete local gallery. `npm run preview:cloudflare` is an alias for `npm run dev`. For frontend-only work, use `npm run dev:frontend`; it does not run the API. See the [deployment guide](docs/cloudflare-deployment.md) before using `npm run deploy`, particularly with your own account or domain. Pushes to `main` deploy through the configured [GitHub workflow](docs/ci-deployment.md) after checks pass.
