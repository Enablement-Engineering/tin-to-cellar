...
for (const width of [1280, 320]) test(`existing community labels survive reload and print without generation at ${width}px`, async ({ context, page }) => {
  ...
  await expect(page.getByRole('status').filter({ hasText: /^2 selected · 2 ready to print$/ })).toBeVisible()
...
test('mixed collection freezes only requested artwork and merges a returned ZIP across tabs without replay', async ({ context, page }) => {
  ...
  const row = page.getByRole('dialog', { name: `Choose artwork for ${third.blend}`, exact: true })
  await expect(row.getByText('No community designs for this blend yet.', { exact: true })).toBeVisible()
  await row.getByRole('button', { name: 'Create with AI', exact: true }).click()
  ...
  await expect(returnTab.getByRole('spinbutton', { name: /^Quantity for / })).toHaveCount(3)
  await returnTab.locator('input[type=file]').first().setInputFiles(returned)
  await returnTab.getByRole('button', { name: 'Show 1 already saved design', exact: true }).click()
  ...
  await expect(page.getByRole('status').filter({ hasText: /^3 selected · 3 ready to print$/ })).toBeVisible()
  expect(contributions).toHaveLength(1)
  expect(requests.some(url => url.includes('example.com/DO-NOT-FETCH'))).toBe(false)
})

for (const width of [1280, 320]) test(`home example pack can replace an existing selection, and print reset requires confirmation at ${width}px`, async ({ context, page }) => {
  ...
  const reviewDialog = page.getByRole('dialog', { name: 'Add your new labels' })
  await expect(reviewDialog).toBeVisible()
  await expect(reviewDialog.getByRole('button', { name: 'Cancel import', exact: true })).toBeInViewport()
  await expect(reviewDialog.getByRole('heading', { name: 'Add your new labels' })).toBeFocused()
  await page.getByText('Start over with this pack instead', { exact: true }).click()
  ...
  const resetDialog = page.getByRole('dialog', { name: 'Reset all 10 labels and start over?' })
  await expect(resetDialog.getByRole('button', { name: 'Cancel', exact: true })).toBeFocused()
  expect((await new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa', 'wcag21aa']).analyze()).violations).toEqual([])
  await page.keyboard.press('Escape')
  await expect(resetDialog).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Reset labels', exact: true })).toBeFocused()
  await expect(page.getByRole('button', { name: 'Print 10 labels', exact: true })).toBeEnabled()
  await page.getByRole('button', { name: 'Reset labels', exact: true }).click()
  await page.getByRole('dialog').getByRole('button', { name: 'Reset labels', exact: true }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
...
for (const width of [1280, 320]) test(`blend additions keep an accurate visible selection count and search focus at ${width}px`, async ({ page }) => {
  ...
  for (let index = 1; index <= 3; index++) {
    ...
    await expect(page.getByRole('status').filter({ hasText: `Test custom blend ${index} added to your AI creation list · ${index} ${index === 1 ? 'blend' : 'blends'} selected.` })).toBeVisible()
    await expect(input).toBeFocused()
  }
  const summary = page.locator('[aria-label="Selection summary"]')
  await expect(summary).toContainText('3 selected · 0 ready to print · 3 to create with AI')
