...
test('instruction download failure recovers after deliberate reload without losing saved labels or focus', async ({ page }) => {
  ...
  const artworkChoice = page.getByRole('dialog', { name: 'Choose artwork for My saved blend', exact: true })
  await artworkChoice.getByRole('button', { name: 'Create with AI', exact: true }).click()
  await expect(artworkChoice).toBeHidden()
  await expect(page.getByRole('heading', { name: 'My saved blend', exact: true })).toBeVisible()
