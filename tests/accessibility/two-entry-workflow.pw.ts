...
for (const width of [1280, 320]) test(`manual artwork choice saves only ready or AI labels at ${width}px`, async ({ context, page }) => {
  await library(context)
  await page.setViewportSize({ width, height: 900 })
  await page.goto('/labels/create')
  const input = page.getByRole('combobox', { name: 'Add a blend' })
  const select = async (entry: Blend) => {
    await input.fill(`${entry.maker} ${entry.blend}`)
    await page.getByRole('option', { name: `${entry.blend} by ${entry.maker}`, exact: true }).click()
    return page.getByRole('dialog', { name: `Choose artwork for ${entry.blend}`, exact: true })
  }
  let dialog = await select(blends[0])
  await expect(dialog.getByRole('heading', { name: `Choose artwork for ${blends[0].blend}` })).toBeFocused()
  expect(await savedRows(page)).toHaveLength(0)
  await dialog.getByRole('button', { name: 'Cancel', exact: true }).focus()
  await page.keyboard.press('Tab')
  await expect(dialog.getByRole('button', { name: 'Create with AI', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('button', { name: 'Cancel', exact: true })).toBeFocused()
  await dialog.getByRole('button', { name: 'Cancel', exact: true }).click()
  await expect(input).toBeFocused()
  expect(await savedRows(page)).toHaveLength(0)
  dialog = await select(blends[0])
  await page.keyboard.press('Escape')
  await expect(dialog).toHaveCount(0)
  await expect(input).toBeFocused()
  dialog = await select(blends[0])
  await dialog.getByRole('button', { name: 'Use this design', exact: true }).first().click()
  await expect(dialog).toHaveCount(0)
  await expect(input).toBeFocused()
  await expect(page.getByRole('region', { name: 'Ready to print', exact: true }).getByRole('article', { name: blends[0].blend })).toBeVisible()
  dialog = await select(blends[1])
  await dialog.getByRole('button', { name: 'Create with AI', exact: true }).click()
  await expect(dialog).toHaveCount(0)
  await expect(input).toBeFocused()
  await expect(page.getByRole('region', { name: 'To create with AI', exact: true }).getByRole('article', { name: blends[1].blend })).toBeVisible()
  const rows = await savedRows(page)
  expect(rows).toHaveLength(2)
  expect(rows[0]).toMatchObject({ createRequested: false, designId: expect.any(String) })
  expect(rows[1]).toMatchObject({ createRequested: true, designId: null })
  await expect(page).toHaveURL(/\/labels\/create$/)
  expect((await new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa', 'wcag21aa']).analyze()).violations).toEqual([])
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await page.screenshot({ path: `test-results/manual-label-groups-${width}.png`, fullPage: true })
  await page.reload()
  await expect(page.getByRole('article')).toHaveCount(2)
  expect(await savedRows(page)).toEqual(rows)
})

test('manual community download failure adds nothing and allows retry', async ({ context, page }) => {
  await library(context)
  await context.route('**/api/gallery/v1/labels/*/pack', route => route.fulfill({ status: 503 }))
  await page.goto('/labels/create')
  await page.getByRole('combobox', { name: 'Add a blend' }).fill(`${blends[0].maker} ${blends[0].blend}`)
  await page.getByRole('option', { name: `${blends[0].blend} by ${blends[0].maker}`, exact: true }).click()
  const dialog = page.getByRole('dialog')
  await dialog.getByRole('button', { name: 'Use this design', exact: true }).first().click()
  await expect(dialog.getByRole('alert')).toContainText('Nothing was added')
  expect(await savedRows(page)).toHaveLength(0)
  await context.unroute('**/api/gallery/v1/labels/*/pack')
  await dialog.getByRole('button', { name: 'Use this design', exact: true }).first().click()
  await expect(dialog).toHaveCount(0)
  expect(await savedRows(page)).toEqual([expect.objectContaining({ designId: expect.any(String), createRequested: false })])
})
