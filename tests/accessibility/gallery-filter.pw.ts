...
test('typing Earl shows the matching designs before a suggestion is selected', async ({ page }) => {
  ...
  await expect(page.getByRole('status').filter({ hasText: '3 labels shown.' })).toBeVisible()
  await input.fill('earl')
  await expect(page.getByRole('status').filter({ hasText: '2 labels shown.' })).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Clear blend' }).click()
  await expect(page.getByRole('status').filter({ hasText: '3 labels shown.' })).toBeVisible()
})
