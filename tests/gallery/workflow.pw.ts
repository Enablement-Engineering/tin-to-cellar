...
test('browser selects artwork, submits privately, reviewer publishes, public user imports and prints', async ({ page, browser, request }) => {
  ...
  await page.getByRole('combobox', { name: 'Blend', exact: true }).selectOption(tobacco.id)
  await page.getByRole('button', { name: 'Search', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Use label' }).first()).toBeVisible()
  expect((await new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa', 'wcag21aa']).analyze()).violations).toEqual([])
  await page.screenshot({ path: 'output/gallery/gallery-public-library.png', fullPage: true })
  const previous = writes.length
  await page.getByRole('button', { name: 'Use label' }).first().click()
  ...
  await page.pdf({ path: 'output/gallery/gallery-print.pdf', format: 'Letter', printBackground: true })
