...
test('curated browser intake and reconciliation preserve public identity and canonical artwork', async ({ browser, request }) => {
  ...
  try {
    ...
    const pending = queue.submissions.find((r: { metadata: { catalogId: string } }) => r.metadata.catalogId === tobacco.id)
    ...
    expect(decode(art).data).toEqual(decode(replacement.png).data)
