...
test('workflow header fits narrow screens and keeps help in the footer', async ({ page }, testInfo) => {
  ...
  for (const width of [320, 375, 600, 768, 1024, 1440]) {
    ...
    if (width === 375 || width === 1024) await page.locator('.site-header').screenshot({ path: testInfo.outputPath(`tin-header-${width}.png`) })
  }
