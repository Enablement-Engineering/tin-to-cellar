...
5. Keep the PR current with freshly fetched main. Resolve conflicts in the branch, preserve current privacy and budget controls, and rerun affected checks. Request AI code review for consequential changes. Read every AI review on the current revision before merging. Address each actionable finding in code or reply with the concrete reason it does not apply, then rerun affected checks after any review fix. AI review is advisory, with no required human approval count, but green CI does not replace reviewing its findings.
...
3. Verify the current PR revision passes every required check, includes current main, and has no unaddressed actionable AI review findings. Recheck reviews after the final push so the review and CI evidence cover the same revision. Merge through the PR, using squash merge by default for a single scoped change. Never push directly to main. Observe the resulting main workflow rather than starting a duplicate release.
