...
`npm run dev` starts Vite and the real Worker together at `http://127.0.0.1:43928`, with persistent local gallery/diagnostics storage and catalog contributions. Frontend edits update live; Wrangler watches Worker source changes. This stack uses a generated local configuration and never serves `dist`, so production builds do not invalidate open development tabs. See the README for adopting an existing populated local gallery. `npm run build` followed by `npm run preview` remains a built-site preview using the root Wrangler configuration and does not initialize the complete local gallery. `npm run dev:frontend` and `npm run preview:frontend` explicitly run only Vite, without API routes. Use them for isolated presentation work, not backend verification.
