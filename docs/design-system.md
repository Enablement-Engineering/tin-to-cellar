...
Import review adds new blends by default and fills unambiguous artwork requests while keeping quantities. New labels have an optional include checkbox. Only matching blends offer artwork choices: Keep current, Use imported artwork, or Keep both, with current and imported previews together. Never offer unrelated blends as replacement targets. Identical designs are skipped. Whole-pack replacement stays under the collapsed Start over with this pack instead disclosure and retains its destructive confirmation. It is the confirmation step; do not add another confirmation dialog. Background lookup must not move focus. After committing an import, focus moves to the quantities heading. Saved browser work survives reload and new tabs, subject to browser storage availability. The explicit Clear saved labels action requires confirmation; downloaded packs remain separate.
...
Browse cards put the blend name in an h2, with the maker beneath it in smaller sans-serif text. Keep artwork, identity, and the add action together; omit the About this design disclosure and date, edition, or other metadata. Preserve the artwork description as image alt text. Name each article from its blend and maker, describe its action with the same identity, and announce that full-resolution artwork links open a new tab. Align actions at the bottom of cards without fixed card heights or truncated names.
