...
One submission action verifies all selected labels together, up to five. The browser sends their drafts with one internal nonce to the batch reservation endpoint, then uploads each selected image separately. The server validates the challenge once and reserves all new entries in one transaction. Admission and storage quotas still count each image. Upload retries reuse the saved reservation without another challenge; an uncertain reservation response is reconciled with the same ID, nonce, and draft before requesting a new check.
...
| `POST /submissions/batch` | Internal nonce + one challenge for `{ submissions: [draft, ...] }`, containing one to five distinct submission IDs. Returns `{ submissions: [receipt, ...] }` in request order. All new reservations and durable quotas commit together. Existing matching reservations replay without a challenge; adding new entries requires fresh verification. |
