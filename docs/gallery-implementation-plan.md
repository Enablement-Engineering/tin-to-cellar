...
The browser constructs `GalleryLabelDraftV1` by picking named fields from a successfully imported label, never spreading the manifest. Fields: `version`, client-generated random submission ID, catalog ID or proposed maker/blend, fixed package/variant plus edition, surface geometry, exactly one validated blank write-in area, selected public references `{url, role}`, expected image SHA/bytes/dimensions, and a versioned sharing acknowledgement. Server accepts exact keys only. Manufacturer/blend strings for known IDs come from the catalog.
...
Meet the current required research shape honestly: `status: limited`; reviewed package/edition or `unknown`; `palette: ["Not recorded in the shared label"]`, empty motifs, remaining required visual-analysis strings `Not recorded in the shared label`; adaptation summary `Community-submitted jar label; private research was not included.` Include reviewed public product sources where available. When none exist, use a fixed `user-provided` source description `Contributor-supplied label; no public package reference was included.` This describes provenance rather than inventing inspection. Do not fabricate a manufacturer reference, model name, original generation timestamp, proof result or complete-research claim. Add a fixed limitation explaining that gallery review does not certify the original AI research. Validate the assembled pack against the unchanged v1 schema and real importer; if the fixture fails, resolve the exporter contract with the CellarPack owner before implementation proceeds.
