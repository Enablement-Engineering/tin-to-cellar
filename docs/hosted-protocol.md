...
All 831 application tests passed with `npm run test -- --maxWorkers=2`, along with the production build, prompt-size limit, release integrity verification, and lint. Lint still reports warnings in the pre-existing untracked generated validator file. All 30 earlier release snapshots are unchanged, and the portable instructions match the new snapshot exactly. This revision is local and has not been deployed or tested in a fresh ChatGPT artwork run. Existing saved handoffs keep their pinned instructions; use **Copy updated instructions for a new chat** to adopt this revision.
