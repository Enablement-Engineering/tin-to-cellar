...
| Low, resolved contract discrepancy | The prose specification promised fallback for unknown optional enum values, but the published schema rejects them, including unknown `defaultPrintIntent.labelQuantityMode`. | Narrowed the compatibility promise to match the existing closed enums. Documented omission versus unknown values, root rejection versus label quarantine, and the separate unavailable-profile warning. Added regression tests; runtime behavior and published schemas remain unchanged. |
