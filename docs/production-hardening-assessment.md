...
| --- | --- | --- |
| Protect main and production | Valid. GitHub returned no rulesets, `Branch not protected` for main, and no protection rules or branch policy for the production environment. The environment already exists in the workflow. The reviewed main commit had no associated PR. | Prepare explicit required-check and release rules. Repository settings remain an owner action. |
...
CodeQL is added as a required deployment dependency. Hosted execution, baseline findings and the separate code-scanning severity rule need GitHub verification. The documented branch workflow requires PRs and checks with zero mandatory human approvals.
