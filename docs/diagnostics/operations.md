...
3. On the first new collection, authorized diagnostics export, or scheduled cleanup, the existing Durable Object copies its retained records to D1 before accepting new diagnostic writes. The migration preserves original receipt dates and 90-day expiry and marks completion only after every write succeeds. Interrupted migrations safely retry. The original records remain under their original expiry. No expired history is recoverable. The admin-token `npm run diagnostics:backfill` script is an optional manual recovery route; it deletes its temporary SQL after execution.
