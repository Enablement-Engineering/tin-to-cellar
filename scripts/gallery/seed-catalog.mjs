...
for (const a of registry.idAliases) {
  statements.push(`INSERT INTO gallery_catalog_aliases (alias_id,catalog_id) VALUES (${quote(a.aliasId)},${quote(a.catalogId)}) ON CONFLICT(alias_id) DO UPDATE SET catalog_id=excluded.catalog_id;`)
  statements.push(`UPDATE gallery_tobaccos SET active=0 WHERE id=${quote(a.aliasId)};`)
}
