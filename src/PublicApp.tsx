...
export default function PublicApp() {
  ...
  return <div className="app-shell tc-grain" onClick={event => {
    ...
    <main id="main-content" ref={main} tabIndex={-1} className={`site-main view-${view}`}>
      ...
      {view === 'not-found' ? <div className="landing-page screen-only"><h1>Page not found</h1><p>This page doesn’t exist.</p><a href="/labels" onClick={routeClick('labels')}>Go to Labels</a></div> : view === 'labels' ? <Landing onNavigate={navigate} busy={busy} onFile={file => handlePack(file, 'example')} /> : view === 'gallery' ? <DeferredPanel><GalleryBrowse onAdd={label => chooseCommunity(label)} selectedIds={Object.values(collection.designs).flatMap(design => design.publicationId ? [design.publicationId] : [])} onPrint={() => navigate('print')} /></DeferredPanel> : view === 'gallery-admin' ? <DeferredPanel><GalleryAdmin /></DeferredPanel> : view === 'create' ? <PreparationWorkspace rows={collection.rows.map(row => ({ ...row, artwork: labels.find(label => label.id === row.id) }))} busy={busy}
