...
it('loads the ZIP only on request and passes the assembled file to the importer', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Import example pack with ten labels' }))
...
it('downloads a ZIP without importing it', async () => {
  ...
  const onFile = vi.fn()
  render(<ExamplePack busy={false} onFile={onFile} />)
  fireEvent.click(screen.getByRole('button', { name: 'Download example ZIP' }))
...
it('offers retry after a failed download and respects an existing import', async () => {
  ...
  expect(onFile).not.toHaveBeenCalled()
  expect(screen.getByRole('button', { name: 'Import example pack with ten labels' })).toBeEnabled()
  rerender(<ExamplePack busy onFile={onFile} />)
  expect(screen.getByRole('button', { name: 'Import example pack with ten labels' })).toBeDisabled()
  expect(screen.getByRole('button', { name: 'Download example ZIP' })).toBeDisabled()
})
