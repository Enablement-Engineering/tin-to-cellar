...
it('preselects a unique match but waits for Add before importing', () => {
  ...
  expect(screen.getByRole('button', { name: 'Add selected blends' })).toBeEnabled()
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Add selected blends' }))
...
it('discards late OCR results after cancellation', async () => {
  ...
  await act(async () => { finish('Orlik\nGolden Sliced') })
  expect(screen.getByRole('status')).toHaveTextContent('Reading cancelled. Choose another file or paste a blend list.')
  expect(screen.queryByRole('combobox')).not.toBeInTheDocument()
  expect(screen.queryByLabelText('Blend list')).not.toBeInTheDocument()
})
 ...
 it('leaves ambiguous matches unselected and lets users skip unique matches', () => {
  ...
  expect(screen.getByRole('button', { name: 'Add selected blends' })).toBeDisabled()
})
