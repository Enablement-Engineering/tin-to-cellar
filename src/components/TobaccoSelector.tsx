...
export function TobaccoSelector({ value, onChange, onChoose, inputRef, disabled, confirmed = false, label = 'Add a blend', hint, actionLabel, multilineHint = 'Choose one blend at a time here.' }: {
  value: string; onChange: (value: string) => void; onChoose: (identity: TobaccoIdentity) => void
  inputRef?: RefObject<HTMLInputElement | null>; disabled?: boolean; confirmed?: boolean; label?: string; hint: string; actionLabel?: string; multilineHint?: string
}) {
  const id = useId()
    ...
    <div className="preparation-add-line"><div className="tobacco-editor">
      <input id={id} ref={inputRef} type="text" role="combobox" aria-autocomplete="list" aria-expanded={visible} aria-controls={visible ? `${id}-options` : undefined} aria-activedescendant={visible && selected >= 0 ? `${id}-${selected}` : undefined} aria-describedby={`${id}-hint`} autoComplete="off" value={value} readOnly={disabled} placeholder="Search or type a blend…" onFocus={() => setOpen(!confirmed)} onBlur={() => { setOpen(false); setActive(-1) }} onChange={event => { onChange(event.target.value); setError(''); setOpen(true); setActive(-1) }} onKeyDown={event => {
        ...
        if ((event.key === 'ArrowDown' || event.key === 'ArrowUp') && value.trim()) { event.preventDefault(); setOpen(true); setActive(current => event.key === 'ArrowDown' ? Math.min(current + 1, matches.length) : current < 0 ? matches.length : Math.max(0, current - 1)) }
          ...
          // Once reviewed, let Enter submit the containing form. Choosing an
          // open suggestion still consumes Enter so it cannot also submit.
          if (confirmed && !visible) return
          event.preventDefault(); if (value.trim()) submit()
        }
      }} onPaste={event => {
      ...
      {visible && <ul id={`${id}-options`} className="tobacco-suggestions" role="listbox" aria-label="Blend suggestions">
        {matches.map((entry, index) => <li id={`${id}-${index}`} key={entry.id} role="option" aria-selected={selected === index} aria-label={`${entry.blend} by ${entry.maker}`} onMouseMove={() => setActive(index)} onMouseDown={event => event.preventDefault()} onClick={() => choose(entry)}><strong>{entry.blend}</strong><span>{entry.maker}</span></li>)}
        <li id={`${id}-${matches.length}`} role="option" aria-selected={selected === matches.length} onMouseMove={() => setActive(matches.length)} onMouseDown={event => event.preventDefault()} onClick={() => choose()}>Use “{value.trim()}” without a catalog match</li>
      </ul>}
