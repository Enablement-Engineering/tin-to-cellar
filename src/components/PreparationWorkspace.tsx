...
function ArtworkChoices({ row, busy, onChooseCommunity }: Pick<PreparationWorkspaceProps, 'busy' | 'onChooseCommunity'> & { row: PreparationRow }) {
  ...
  return <div className="preparation-choices" aria-busy={loading}>
    ...
    {page?.serving && page.labels.length > 0 && <ul className="preparation-designs" aria-label={`Community designs for ${row.blend}`}>{page.labels.map(label => <li key={label.id}>
      <a href={`${API}/labels/${label.id}/artwork`} target="_blank" rel="noreferrer" aria-label={`View ${label.blend}${label.edition ? `, ${label.edition}` : ''} artwork`}><GalleryThumbnail src={`${API}/labels/${label.id}/thumbnail`} alt={label.description} eager={false} /></a>
      <p>{label.edition || 'Community design'}</p>
      <button type="button" className="button secondary" disabled={busy || choosing !== null} onClick={() => void choose(label)}>{choosing === label.id ? 'Adding design…' : 'Use this design'}</button>
    </li>)}</ul>}
  ...
  return <section className="create-workspace preparation-workspace screen-only" aria-labelledby="preparation-title">
    ...
    {rows.length > 0 && <>
      ...
      <div className="preparation-rows">{rows.map(row => <article className="panel preparation-row" key={row.id} aria-labelledby={`row-${row.id}`}>
        ...
        <div className="preparation-create-choice"><button type="button" className="button secondary" aria-pressed={row.createRequested} disabled={busy} onClick={() => onCreate(row.id, !row.createRequested)}>{row.createRequested ? 'Remove from creation request' : 'Create my own'}</button>{row.createRequested && onNotes && <RowNotes row={row} onNotes={onNotes} />}</div>
      </article>)}</div>
    </>}
    {handoff}
