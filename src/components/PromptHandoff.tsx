...
export function PromptHandoff({ prompt, request, completePrompt = prompt, onPrint }: PromptHandoffProps) {
  const payloads = { hosted: prompt, complete: completePrompt, request }
  ...
  return (
    <section className="handoff" aria-labelledby="handoff-title">
      ...
      <div className="handoff-actions"><button className="button primary" type="button" onClick={() => void copy('hosted')}><Icon name="copy" />Copy prompt</button></div>
      ...
      <details className="prompt-preview" open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
        ...
        <div className="handoff-reuse"><p className="field-hint">If your AI cannot read the instructions, copy the complete prompt and paste it into the same chat.</p><button className="handoff-text-action" type="button" onClick={() => void copy('complete')}>Copy complete prompt</button><p className="field-hint">For a chat that already has the instructions, copy just the request. Or download the instructions and attach them with your request.</p><button className="handoff-text-action" type="button" onClick={() => void copy('request')}>Copy request only</button><a href={`data:text/markdown;charset=utf-8,${encodeURIComponent(protocolInstructions())}`} download="tin-to-cellar-instructions.md">Download instructions</a></div>
      </details>
      </div>
    </section>
  )
