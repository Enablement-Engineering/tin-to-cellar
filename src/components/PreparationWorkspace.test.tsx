...
import { PreparationWorkspace, type PreparationWorkspaceProps } from './PreparationWorkspace'
...
it('queries every committed catalog row without the discovery eight-ID cutoff', async () => {
  ...
  await waitFor(() => expect(new Set(vi.mocked(searchExactLabels).mock.calls.filter(([id]) => id.startsWith('catalog-')).map(([id]) => id)).size).toBe(9))
...
it('keeps notes editable while typing and saves only when leaving the field', async () => {
  ...
  render(<PreparationWorkspace {...props()} onNotes={onNotes} rows={[{ id: 'row', catalogId: null, maker: '', blend: 'Custom', createRequested: true }]} />)
