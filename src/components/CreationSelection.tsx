...
export function CreationSelection({ rows, busy, onSave, onClose }: {
  ...
  return <dialog ref={dialog} className="import-review-dialog creation-selection" aria-labelledby="creation-selection-title" aria-describedby="creation-selection-description" onCancel={event => { event.preventDefault(); if (!busy && !saving) onClose() }}>
    <header className="import-review-header"><h2 id="creation-selection-title" ref={heading} tabIndex={-1}>Choose artwork to create</h2><p id="creation-selection-description">Select the blends that need new designs. If a label already has artwork, its current design will be saved but left off the print sheet until you restore or replace it.</p></header>
    <div className="import-review-body">
      <p className="field-hint">Labels that need artwork are checked. Uncheck any you want to find in the community.</p>
      <div className="creation-targets">{rows.map(row => <label key={row.id}><input type="checkbox" checked={selected.includes(row.id)} disabled={busy || saving} onChange={event => setSelected(ids => event.target.checked ? [...ids, row.id] : ids.filter(id => id !== row.id))} /><span><strong>{row.blend}</strong><small>{row.maker}{row.maker ? ' · ' : ''}{row.artwork ? 'Current design saved if selected' : row.previousDesignId ? 'Uncheck to restore previous design' : 'Needs artwork'}</small></span></label>)}</div>
