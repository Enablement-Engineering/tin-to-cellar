...
it('copies and reveals the exact complete payload when its clipboard action fails', async () => {
  ...
  expect(screen.getByRole('link', { name: 'Current hosted instructions' })).toHaveAttribute('href', 'https://tintocellar.com/api/protocol/v1')
})
...
it('offers reusable bundled instructions without the project request or proof access', () => {
  ...
  const link = screen.getByRole('link', { name: /Download complete instructions/ })
