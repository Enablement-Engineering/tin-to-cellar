...
export function FloatingPack({ selected, busy, assembling, onRemove, onClear, onCreate }: {
  ...
  const returnToGallery = () => { setOpen(false); document.getElementById('gallery-title')?.focus({ preventScroll: true }) }
  ...
  return createPortal(<section className="gallery-pack-floating screen-only" aria-label="Your pack" onKeyDown={event => {
    ...
    <button ref={toggle} className="button gallery-pack-toggle" aria-expanded={expanded} aria-controls="gallery-pack-panel" onClick={() => setOpen(!expanded)}>
    ...
    <div id="gallery-pack-panel" className="gallery-pack-tray" hidden={!expanded}>
      <div className="gallery-pack-heading"><h2>Selected labels</h2><button className="button quiet" disabled={assembling} onClick={() => { onClear(); returnToGallery() }}>Clear pack</button></div>
      <ul className="gallery-pack-items">{selected.map(label => <li key={label.id}><span>{label.maker} · {label.blend}</span><button className="button quiet" disabled={assembling} aria-label={`Remove ${label.maker} ${label.blend} from pack`} onClick={() => { onRemove(label.id); if (selected.length === 1) returnToGallery(); else toggle.current?.focus({ preventScroll: true }) }}>Remove</button></li>)}</ul>
