...
it('keeps selection through browsing and remount, removes entries and prints the whole selection', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'Your pack · 2 labels' })).toBeInTheDocument()
  ...
  await screen.findByRole('heading', {name: 'Your pack · 2 labels'})
  fireEvent.click(screen.getByRole('button', {name: 'Remove Maker One from pack'}))
  expect(screen.getByRole('heading', {name: 'Your pack · 1 label'})).toBeInTheDocument()
