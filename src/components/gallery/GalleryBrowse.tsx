...
export function GalleryBrowse({ onUse }: { onUse: (file: File) => Promise<void> }) {
  ...
  return <section className="gallery-page screen-only" aria-labelledby="gallery-title"><header className="page-heading"><h1 id="gallery-title" tabIndex={-1}>Community labels</h1><p>Find a design, choose your quantities, and print for personal cellaring.</p></header>
    ...
    {config?.serving && <><div className="gallery-filters"><GalleryBlendSearch onChange={changeFilter} /><label>Label shape<select aria-label="Label shape" defaultValue="circle-2.5"><option value="circle-2.5">2.5-inch circle</option></select></label></div>
