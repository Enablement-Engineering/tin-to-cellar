  ...
  return <section className="gallery-page screen-only" aria-labelledby="gallery-title">
    ...
    {config?.serving && <>
      ...
      <div className="gallery-grid" role="region" aria-label="Label results" aria-busy={busy}>{shown.map((label, index) => <article className="gallery-card" key={label.id} aria-labelledby={`gallery-blend-${label.id} gallery-maker-${label.id}`}>
        <button type="button" className="gallery-artwork-link" aria-haspopup="dialog" aria-label={`View full-resolution ${label.blend} by ${label.maker} artwork`} onClick={() => setPreview(label)}><GalleryThumbnail preview={label.previewDataUrl} src={`${API}/labels/${label.id}/thumbnail`} alt={label.altText || `${label.maker} ${label.blend} artwork`} eager={index < 4} /></button>
