...
export function GallerySubmission({ labels }: { labels: ImportedCellarLabel[] }) {
  ...
  const start = async () => {
    if (inFlight.current || active !== null) return
    inFlight.current = true
    setError(''); setBusy(true)
    try {
      const result: Attempt[] = []
      const key = hex(crypto.getRandomValues(new Uint8Array(32)).buffer)
      for (const item of labels.filter(item => selected.includes(item.id))) result.push({ label: item, draft: await buildDraft(item, choices[item.id] ?? initial(item), crypto.randomUUID()), key })
  ...
  return <section className="panel gallery-submission screen-only" aria-labelledby="share-labels-title">
    ...
    <div className="gallery-grid">{labels.map(item => { const choice = choices[item.id] ?? initial(item), checked = selected.includes(item.id); return <article className="gallery-card" key={item.id}>
      ...
      <label className="gallery-check"><input type="checkbox" checked={checked} disabled={busy || active !== null || (!checked && selected.length >= MAX_SUBMISSION_LABELS)} onChange={() => setSelected(old => checked ? old.filter(id => id !== item.id) : [...old, item.id])} />Share {item.label.maker} {item.label.blend}</label>
