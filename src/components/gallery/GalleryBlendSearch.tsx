...
export function GalleryBlendSearch({ onChange }: { onChange: (catalogId: string, pending: boolean) => void }) {
  ...
  return <div className="field tobacco-picker gallery-blend-search">
    ...
    <div className="tobacco-editor">
      <input type="text" id={id} role="combobox" aria-autocomplete="list" aria-expanded={showOptions} aria-controls={showOptions ? `${id}-options` : undefined} aria-activedescendant={showOptions && active >= 0 ? `${id}-option-${active}` : undefined} aria-describedby={`${id}-hint`} autoComplete="off" placeholder="Search by maker or blend…" value={query}
    ...
    <span className="field-hint" id={`${id}-hint`}>{showOptions ? matches.length ? 'Choose a blend from the suggestions.' : 'No matching blends. Try another name.' : 'Leave blank to browse all blends.'}</span>
    {query && <button className="clear-tobaccos" type="button" onClick={() => { setQuery(''); setOpen(false); setActive(-1); onChange('', false) }}>Clear blend</button>}
  </div>
}
