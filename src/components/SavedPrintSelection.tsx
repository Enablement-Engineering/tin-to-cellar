...
export function SavedPrintSelection({ rows, previewUnavailable, onChoose }: {
  ...
  return <section className="panel saved-print-selection screen-only" aria-labelledby="saved-print-selection-title">
    ...
    <p>{rows.length} {rows.length === 1 ? 'label is' : 'labels are'} saved. {previewUnavailable ? 'The preview could not load. Reload to try again.' : 'These blends still need artwork. Choose community designs or import a finished label ZIP to print them.'}</p>
