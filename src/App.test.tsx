...
it('keeps past import warnings separate from suspended print readiness', async () => {
  window.history.replaceState({}, '', '/labels/print')
  importer.mockResolvedValue({ ...ready(), issues: [{ severity: 'warning', code: 'LIMITED_RESEARCH', message: 'Research was limited.' }] })
  render(<App />)
  await upload()
  ...
  fireEvent.click(await screen.findByRole('button', { name: 'Create with ChatGPT' }))
  expect(screen.queryByText('1 label ready.')).not.toBeInTheDocument()
  fireEvent.click(screen.getByRole('button', { name: 'I already have a finished ZIP' }))
  expect(screen.queryByText('ZIP needs repair')).not.toBeInTheDocument()
  expect(screen.queryByText('Import needs repair')).not.toBeInTheDocument()
...
it.each(['/labels/print', '/labels/help'])('links to the supported blank label sheets on %s without leaving the workspace', async path => {
  window.history.replaceState({}, '', path)
  render(<App />)
  const link = await screen.findByRole('link', { name: /^Buy label sheets from Avery/ })
  expect(screen.getAllByRole('link', { name: /^Buy label sheets from Avery/ })).toHaveLength(1)
...
it('links the landing print step to paper guidance without a shopping link', async () => {
...
describe('home, prompt, print, and help navigation', () => {
  it('preserves a saved old prompt on reload and lets a new chat use updated instructions', async () => {
    ...
    render(<App />)
    const copy = await screen.findByRole('button', { name: 'Copy instructions' })
  ...
  it('starts on the landing page and keeps committed labels when returning through home', async () => {
    window.history.replaceState({}, '', '/labels')
    render(<App />)
    expect(screen.getByRole('heading', { name: 'Keep the character of the tin.' })).toBeInTheDocument()
    ...
    expect(workflow.getAllByRole('link').map(link => link.textContent)).toEqual(['Gallery', 'Your labels', 'Print labels'])
    ...
    fireEvent.click(within(screen.getByRole('navigation', { name: 'Workflow' })).getByRole('link', { name: 'Your labels' }))
    ...
    fireEvent.click(within(screen.getByRole('main')).getByRole('button', { name: 'Resume your labels' }))
  ...
  it('imports from an unavailable gallery while preserving saved creation requests and their count', async () => {
    render(<App />)
    await addBlend('Escudo Navy Deluxe')
    const before = await savedCollection()
    const navigation = within(screen.getByRole('navigation', { name: 'Workflow' }))
    expect(navigation.getByRole('link', { name: 'Your labels' })).toHaveAccessibleDescription('1 saved label')
    fireEvent.click(navigation.getByRole('link', { name: 'Gallery' }))
    fireEvent.click(await screen.findByRole('button', { name: 'Import a label ZIP' }))
    expect(window.location.pathname).toBe('/labels/print')
    expect(await screen.findByLabelText('Label ZIP')).toBeVisible()
    expect(importer).not.toHaveBeenCalled()
    expect((await savedCollection())?.rows).toEqual(before?.rows)
  })
  it('opens ZIP import directly from home without starting a creation request', async () => {
    window.history.replaceState({}, '', '/labels')
    render(<App />)
    fireEvent.click(screen.getByRole('button', { name: 'Import a label ZIP' }))
    expect(window.location.pathname).toBe('/labels/print')
    expect(await screen.findByLabelText('Label ZIP')).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Copy instructions/ })).not.toBeInTheDocument()
    expect(importer).not.toHaveBeenCalled()
  })
  it('confirms clearing from home, preserves labels on cancel, and stays empty after reload', async () => {
    window.history.replaceState({}, '', '/labels/print')
    importer.mockResolvedValue(ready([imported('Blend A'), imported('Blend B')]))
    const app = render(<App />)
    await upload()
    fireEvent.change(await screen.findByLabelText('Quantity for Blend A'), { target: { value: '3' } })
    await waitFor(() => expect(screen.getByRole('button', { name: 'Print 4 labels' })).toBeEnabled())
    fireEvent.click(screen.getByRole('link', { name: 'Tin to Cellar home' }))
    const before = await savedCollection()
    const clear = screen.getByRole('button', { name: 'Clear labels' })
    clear.focus()
    fireEvent.click(clear)
    const dialog = screen.getByRole('dialog', { name: 'Clear all 2 labels and start over?' })
    expect(within(dialog).getByRole('button', { name: 'Cancel' })).toHaveFocus()
    expect(dialog).toHaveTextContent('Downloaded ZIP files are unchanged.')
    fireEvent.click(within(dialog).getByRole('button', { name: 'Cancel' }))
    expect(clear).toHaveFocus()
    expect(await savedCollection()).toEqual(before)
    fireEvent.click(clear)
    fireEvent.click(within(screen.getByRole('dialog')).getByRole('button', { name: 'Clear labels' }))
    await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument())
    expect(screen.queryByRole('button', { name: 'Resume your labels' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Clear labels' })).not.toBeInTheDocument()
    expect(screen.getByRole('main')).toHaveFocus()
    expect(window.location.pathname).toBe('/labels')
    expect(await savedCollection()).toMatchObject({ rows: [], designs: {}, receipts: [], handoff: null, printSettings: { page: 0, firstSlot: 1, offset: { x: 0, y: 0 } } })
    app.unmount()
    render(<App />)
    await waitFor(() => expect(screen.getByRole('button', { name: 'Try the example pack' })).toBeEnabled())
    expect(screen.queryByRole('button', { name: 'Resume your labels' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Clear labels' })).not.toBeInTheDocument()
  })
  it('keeps the clear confirmation and saved work available when storage fails', async () => {
    render(<App />)
    await addBlend('My test blend')
    fireEvent.click(screen.getByRole('link', { name: 'Tin to Cellar home' }))
    const before = await savedCollection()
    fireEvent.click(screen.getByRole('button', { name: 'Clear labels' }))
    const dialog = screen.getByRole('dialog', { name: 'Clear 1 label and start over?' })
    vi.spyOn(IDBObjectStore.prototype, 'put').mockImplementationOnce(() => { throw new DOMException('Full', 'QuotaExceededError') })
    fireEvent.click(within(dialog).getByRole('button', { name: 'Clear labels' }))
    expect(await within(dialog).findByRole('alert')).toHaveTextContent('Your previously saved labels are still available')
    expect(await savedCollection()).toEqual(before)
    expect(within(dialog).getByRole('button', { name: 'Clear labels' })).toBeEnabled()
    fireEvent.click(within(dialog).getByRole('button', { name: 'Clear labels' }))
    await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument())
  ...
  it('requires committed creation targets and saves the exact prompt before copying', async () => {
    ...
    expect(screen.queryByRole('button', { name: /Copy instructions/ })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Add blend' }))
    expect(screen.getByRole('dialog', { name: 'Choose artwork for Unlisted Family Tobacco' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Copy instructions/ })).not.toBeInTheDocument()
    ...
    const copy = await screen.findByRole('button', { name: 'Copy instructions' })
    fireEvent.click(screen.getByText('Design notes · optional'))
    const notes = screen.getByRole('textbox', { name: 'Requests for Unlisted Family Tobacco optional' })
    fireEvent.change(notes, { target: { value: 'Keep the original gold border' } })
    fireEvent.blur(notes)
    await waitFor(async () => expect((await savedCollection())?.rows[0].notes).toBe('Keep the original gold border'))
    ...
    expect(vi.mocked(navigator.clipboard.writeText).mock.calls[0][0]).toContain('Keep the original gold border')
  })
  it('returns to the landing page when browser Back restores the initial label URL', async () => {
    ...
    fireEvent.click(within(screen.getByRole('navigation', { name: 'Workflow' })).getByRole('link', { name: 'Your labels' }))
    window.history.back()
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Keep the character of the tin.' })).toBeInTheDocument())
    expect(window.location.pathname).toBe('/labels')
    expect(screen.getByRole('main')).toHaveFocus()
    expect(window.scrollTo).toHaveBeenLastCalledWith({ top: 0, left: 0 })
  })
  ...
  it('opens a direct print path without importing or fetching a file', async () => {
    window.history.replaceState({}, '', '/labels/print')
    render(<App />)
    expect(screen.getByText('Loading your saved labels…')).toHaveAttribute('role', 'status')
    expect(screen.queryByLabelText('Label ZIP')).not.toBeInTheDocument()
    expect(await screen.findByLabelText('Label ZIP')).toBeInTheDocument()
  ...
  it('preserves paper position, alignment, and preview page when switching screens', async () => {
    ...
    fireEvent.click(screen.getByRole('link', { name: /Your labels/i }))
    fireEvent.click(screen.getByRole('link', { name: 'How it works' }))
    fireEvent.click(screen.getByRole('link', { name: 'Tin to Cellar home' }))
    fireEvent.click(screen.getByRole('link', { name: /Print labels/i }))
  ...
  it('offers repair for a partial archive even when every label can print', async () => {
    ...
    expect(within(screen.getByRole('complementary', { name: 'Import needs attention' })).getByRole('button', { name: 'Copy repair request' })).toBeInTheDocument()
  })
  ...
  it('preserves existing labels, quantities and prior receipts when an additional ZIP is rejected', async () => {
    ...
    await within(screen.getByRole('complementary', { name: 'Import needs attention' })).findByRole('button', { name: 'Copy repair request' })
    expect(screen.getByLabelText('Quantity for Blend A')).toHaveValue(3)
    expect(URL.revokeObjectURL).not.toHaveBeenCalled()
    fireEvent.click(within(screen.getByRole('complementary', { name: 'Import needs attention' })).getByRole('button', { name: 'Copy repair request' }))
  ...
  it('keeps incompatible artwork off the printed sheet and provides repair instructions', async () => {
    ...
    expect(screen.getByText('1 label was excluded from your last ZIP import.')).toBeVisible()
    fireEvent.click(screen.getByText('View details'))
    expect(within(screen.getByRole('complementary', { name: 'Import needs attention' })).getByRole('button', { name: 'Copy repair request' })).toBeInTheDocument()
...
it('opens the privacy page from the footer and directly by path', async () => {
  ...
  fireEvent.click(within(screen.getByRole('contentinfo')).getByRole('link', { name: 'Privacy and data choices' }))
...
it('redirects the site root to Labels using real links', () => {
  ...
  render(<App />)
  expect(within(screen.getByRole('contentinfo')).getByRole('link', { name: 'Privacy and data choices' })).toHaveAttribute('href', '/privacy')
  expect(window.location.pathname).toBe('/labels')
  expect(screen.getByRole('heading', { name: 'Keep the character of the tin.' })).toBeInTheDocument()
...
it('opens import privacy details without discarding the imported pack', async () => {
  ...
  vi.mocked(URL.revokeObjectURL).mockClear()
  fireEvent.click(within(screen.getByRole('main')).getByRole('link', { name: 'Privacy and data choices' }))
...
it('adds trusted examples without collecting their diagnostics or erasing prior local work', async () => {
  ...
  try {
    ...
    // History stays on the previous user upload; the trusted example is excluded.
    expect(screen.getByRole('button', { name: 'View shared diagnostics' })).toBeInTheDocument()
    const historyOptions = within(screen.getByLabelText('Previous import')).getAllByRole('option')
...
it('identifies import history without changing the selected print job, and resets from Print labels', async () => {
  window.history.replaceState({}, '', '/labels/print')
  importer.mockResolvedValueOnce(ready([imported('Blend A'), imported('Excluded label', 'square')])).mockResolvedValueOnce(ready([imported('Blend B')]))
  render(<App />)
  await upload()
  await waitFor(() => expect(screen.getByRole('button', { name: 'Print 1 label' })).toBeEnabled())
  ...
  fireEvent.click(screen.getByText('Add labels from a ZIP'))
  expect(screen.getByText('Import history').closest('details')).not.toHaveAttribute('open')
  fireEvent.click(screen.getByText('Import history'))
  expect(screen.getByLabelText('Previous import')).toBeVisible()
  expect(screen.queryByText(/selected for printing|Choosing a report keeps/)).not.toBeInTheDocument()
  expect(screen.queryByRole('complementary', { name: 'Import needs attention' })).not.toBeInTheDocument()
  ...
  expect(screen.queryByRole('complementary', { name: 'Import needs attention' })).not.toBeInTheDocument()
  expect(within(history.closest('details')!).getByRole('button', { name: 'Copy repair request' })).toBeVisible()
  expect(screen.getByLabelText('Quantity for Blend A')).toHaveValue(1)
  expect(screen.getByLabelText('Quantity for Blend B')).toHaveValue(1)
  ...
  await waitFor(() => expect(screen.getByRole('button', { name: 'Print 2 labels' })).toBeEnabled())
  const resetDialog = screen.getByRole('dialog', { name: 'Reset all 2 labels and start over?' })
  ...
  expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
  fireEvent.click(screen.getByRole('button', { name: 'Reset labels' }))
  fireEvent.click(within(screen.getByRole('dialog')).getByRole('button', { name: 'Reset labels' }))
...
it('returns to the saved print job when Cancel import is clicked, preserving quantities and alignment', async () => {
  ...
  await upload()
  fireEvent.change(await screen.findByLabelText('Quantity for Blend A'), { target: { value: '3' } })
  fireEvent.change(screen.getByLabelText('Start at slot'), { target: { value: '4' } })
  fireEvent.change(screen.getByLabelText('Horizontal adjustment'), { target: { value: '0.12' } })
  fireEvent.change(screen.getByLabelText('Vertical adjustment'), { target: { value: '-0.08' } })
  await waitFor(() => expect(screen.getByRole('button', { name: 'Print 3 labels' })).toBeEnabled())
  await waitFor(async () => expect((await savedCollection())?.receipts[0].delivery).toBe('sent'))
  const before = (await savedCollection())!

  await upload(true)
  const dialog = await screen.findByRole('dialog', { name: 'Add your new labels' })
  const cancel = within(dialog).getByRole('button', { name: 'Cancel import' })
  await waitFor(() => expect(cancel).toBeEnabled())
  fireEvent.click(cancel)

  await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument())
  expect(await screen.findByLabelText('Quantity for Blend A')).toHaveValue(3)
  expect(screen.getByLabelText('Start at slot')).toHaveValue('4')
  expect(screen.getByLabelText('Horizontal adjustment')).toHaveValue(0.12)
  expect(screen.getByLabelText('Vertical adjustment')).toHaveValue(-0.08)
  expect(screen.getByRole('button', { name: 'Print 3 labels' })).toBeEnabled()
  expect(screen.getByRole('img', { name: 'Slot 4: Test Maker — Blend A' })).toBeVisible()
  expect(screen.queryByLabelText('Quantity for Unsaved blend')).not.toBeInTheDocument()
  const after = (await savedCollection())!
  expect(after.rows).toEqual(before.rows)
  expect(after.designs).toEqual(before.designs)
  expect(after.printSettings).toEqual(before.printSettings)
  expect(after.receipts).toEqual(before.receipts)
})

it('shows saved requests after Cancel import when no selected blend has artwork yet', async () => {
  importer.mockResolvedValueOnce(ready([imported('Unsaved blend')]))
  const initial = render(<App />)
  await addBlend('Awaiting first artwork')
  await addBlend('Awaiting second artwork')
  const before = (await savedCollection())!
  fireEvent.click(screen.getByRole('link', { name: 'Print labels' }))
  await upload(true)
  const dialog = await screen.findByRole('dialog', { name: 'Add your new labels' })
  ...
  expect(await screen.findByRole('heading', { name: 'Your saved selection' })).toBeVisible()
  expect(screen.getByText('Awaiting first artwork')).toBeVisible()
  expect(screen.getByText('Awaiting second artwork')).toBeVisible()
  expect(screen.getByRole('button', { name: 'Choose artwork' })).toBeVisible()
  expect(screen.getByRole('button', { name: 'Reset labels' })).toBeVisible()
  expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
  expect(screen.queryByLabelText('Quantity for Unsaved blend')).not.toBeInTheDocument()
  expect((await savedCollection())?.rows).toEqual(before.rows)
  expect((await savedCollection())?.receipts).toEqual(before.receipts)

  initial.unmount()
  render(<App />)
  expect(await screen.findByRole('heading', { name: 'Your saved selection' })).toBeVisible()
  fireEvent.click(screen.getByRole('button', { name: 'Choose artwork' }))
  expect(await screen.findByRole('heading', { name: 'Awaiting first artwork' })).toBeVisible()
  expect(screen.getByRole('heading', { name: 'Awaiting second artwork' })).toBeVisible()
})
