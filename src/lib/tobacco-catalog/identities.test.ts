...
describe('permanent catalog identity', () => {
  it('freezes active runtime IDs and resolves the retired Capstan duplicate', () => {
    expect(TOBACCO_CATALOG).toHaveLength(1511)
    ...
    expect(new Set(registry.entries.map(e => e.id)).size).toBe(1511)
    expect(resolveTobaccoId('capstan-original-flake-blue')).toBe(resolveTobaccoId('capstan-original-flake'))
    expect(findExactTobacco('Capstan', 'Original Flake Blue')).toBe(resolveTobaccoId('capstan-original-flake'))
  })
