...
export type CollectionRow = Required<RequestInput> & { id: string; revision: number; quantity: number; designId: string | null; createRequested: boolean }
...
export type ImportReceipt = { id: string; title: string; createdAt: string; protocolContext: ProtocolContext; repairPrompt: string; issues: ValidationIssue[]; quarantined: { id: string; reason: string }[]; contribution: Contribution | null; delivery: ReceiptDelivery; knownGalleryHashes?: string[] }
