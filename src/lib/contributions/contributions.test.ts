...
const feedback = { format: 'tin-to-cellar/feedback', schemaVersion: '0.2.0', protocolRevision: '0.0.14', request: { labelCount: 1, shape: 'circle' }, outcome: 'complete', steps: [{ stage: 'research', status: 'passed', attempts: 1 }], issues: [] }
...
it('matches the existing strict feedback validator for invalid field shapes and bounds', () => {
  for (const candidate of [feedback, ...[13, '0.0', '00.0.14', '0.0.14/private', '9999999.0.0', null].map(protocolRevision => ({ ...feedback, protocolRevision })), { ...feedback, outcome: 'invented' }, { ...feedback, steps: [{ stage: 'research', status: 'passed', attempts: 2000 }] }, { ...feedback, request: { labelCount: 1, shape: 'circle', email: 'private' } }, { ...feedback, issues: [{ code: 'geometry', stage: 'proof', resolved: 'yes' }] }, null, {}]) {
...
it('looks up known blend sources and asks for link revalidation without sending custom requests in URLs', () => {
  ...
  expect(prompt).not.toContain(`/api/labels/sources?catalogId=${entry.id}`)
