...
import { PREFERENCE_KEY, PROGRESS_KEY } from './preferences'
import { startProgress, importProgress, printProgress, pruneProgress } from './progress'
...
beforeEach(async () => {
  ...
  await initializeDemandCollection()
})
...
it('matches the saved request locally and sends only coarse milestone counters once', async () => {
  ...
  await startProgress(before.id, before.handoff!)
  await startProgress(before.id, before.handoff!)
  await importProgress(before, after, incoming)
  await importProgress(before, after, incoming)
  await printProgress(after, after.rows.map(r => r.id))
  await printProgress(after, after.rows.map(r => r.id))
...
it('does not infer progress from gallery artwork, changed requests, or a subset of printed rows', async () => {
  const { before, after, incoming } = await fixture()
  await startProgress(before.id, before.handoff!)
  await importProgress(before, after, { ...incoming, receipt: { ...incoming.receipt, origin: 'gallery' } })
  await importProgress({ ...before, handoff: { ...before.handoff!, id: 'different' } }, after, incoming)
...
it('does not resurrect local metadata after off/on while a receipt is pending', async () => {
  ...
  const pending = startProgress(before.id, before.handoff!)
...
it('expires local records on the next check and stays silent without opt-in or browser locks', async () => {
  ...
  pruneProgress(); await Promise.resolve(); await Promise.resolve()
...
it('suppresses simultaneous starts in different callers through the browser lock', async () => {
  const { before } = await fixture()
  await Promise.all([startProgress(before.id, before.handoff!), startProgress(before.id, before.handoff!)])
  expect(sent()).toHaveLength(1)
})
...
it('does not complete a partial import after a previously matched target changes', async () => {
  ...
  await importProgress(before, partial, a)
  ...
  await importProgress(changed, complete, b)
  expect(sent().map(r => r.milestone)).toEqual(['started'])
})
it('does not match a print after an imported request target changes', async () => {
  ...
  await printProgress(changed, changed.rows.map(r => r.id))
  expect(sent().map(r => r.milestone)).toEqual(['started','imported'])
})
it('does not send an already-read old attempt after consent changes during matching', async () => {
  ...
  const interleaved = { ...before, get rows() {
    if (!changed) { changed = true; setDemandPreference(false); setDemandPreference(true) }
    return before.rows
  } }
  await importProgress(interleaved, after, incoming)
  expect(sent().map(r => r.milestone)).toEqual(['started'])
})
